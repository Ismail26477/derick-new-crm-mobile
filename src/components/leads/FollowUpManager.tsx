...
export function FollowUpManager({ leadId, leadName }: FollowUpManagerProps) {
  ...
  useEffect(() => {
    const loadFollowUps = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(`/api/followups/lead/${leadId}`)
        if (response.ok) {
  ...
  const handleCreateFollowUp = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast({
          title: "Error",
          description: errorData.message || "Failed to create follow-up",
          variant: "destructive",
        })
        return
      }
      ...
      toast({ title: "Follow-up created", description: "Follow-up scheduled successfully" })
    } catch (error) {
  ...
  const handleCompleteFollowUp = async () => {
    ...
    try {
      const response = await fetch(`/api/followups/${selectedFollowUp.id}/complete`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleCancelFollowUp = async (followUpId: string) => {
    try {
      const response = await fetch(`/api/followups/${followUpId}/cancel`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      })
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {pendingFollowUps.length} pending • {completed.length} completed
          </p>
        </div>
